(function () {
  ...
  async function renderPdfInViewer(pdf) {
    ...
    try {
      ...
      try {
        ...
        renderPdfWithBrowserFallback(bytes, pdf, pdfError);
      }
    } catch (error) {
      if (renderId !== activePdfRenderId) return;
  ...
  function renderPdfWithBrowserFallback(bytes, pdf, error) {
    revokeActivePdfObjectUrl();
    activePdfObjectUrl = URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' }));

    const notice = createViewerMessage(
      `这份 PDF 结构不标准，PDF.js 无法解析，已切换到兼容预览模式。原始错误：${error?.message || '未知错误'}`,
      false
    );
    notice.classList.add('fallback-notice');
    ...
    elements.pdfViewer.replaceChildren(notice, iframe);
  }
